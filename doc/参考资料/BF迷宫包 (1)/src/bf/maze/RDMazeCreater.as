package bf.maze{
	//递归分割法迷宫生成器
	//递归分割法属于非图论型搜索，生成的迷宫较为随机，但直路多容易解
	public class RDMazeCreater{
		...
		//block不可通行的表示法
		//noBlock可通行的表示法
		public function RDMazeCreater(haveBorder:Boolean=false,minArea:int=0,block:Object=true,noBlock:Object=false){
		...
		public function createMaze(mazeWidth:int,mazeHeight:int,seed:uint=0,minArea:int=0):Array{
			...
			for(var x:int=0;x<=mazeWidth;x++){
				...
				for(var y:int=0;y<=mazeHeight;y++){
					if(_haveBorder && (x==0 || x==mazeWidth || y==0 || y==mazeHeight)){
						_mazeMap[x][y]=_block;
					}else{
						_mazeMap[x][y]=_noBlock;
					}
				}
			}
		...
		private function recursiveDivision(left:int,right:int,top:int,bottom:int){
			//要分隔区域的尺寸
			var areaWidth:int=right-left;
			var areaHeight:int=bottom-top;
			...
			//产生随机分割点
			if(areaWidth>1){
				dx=left+1+Math.floor(rand()*(areaWidth/2))*2;
			}
			if(areaHeight>1){
				dy=top+1+Math.floor(rand()*(areaHeight/2))*2;
			}
			...
			if(dx!=-1 && dy!=-1){
				...
				_mazeMap[left+Math.floor(rand()*(areaWidth/2+1))*2][dy]=_noBlock;
				recursiveDivision(left,right,top,dy-1);
				recursiveDivision(left,right,dy+1,bottom);
			}else if(dy==-1){
				_mazeMap[dx][top+Math.floor(rand()*(areaHeight/2+1))*2]=_noBlock;
